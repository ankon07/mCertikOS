...
pmem_init(unsigned int mbi_addr)
{
  ...
  unsigned int nps;
  unsigned int row_count; //No of row in memory map table
  unsigned int st_add; 
  unsigned int length;
  ...
  if(row_count == 0){
    ...
    st_add = get_mms(row_count - 1);
    length = get_mml(row_count - 1);
    nps = (st_add + length -1)/PAGESIZE;
  }
  ...
  for(i = 0; i < row_count; i++){
    ...
    perm = is_usable(i);
    perm = perm == 1? 2: 0;
    page_idx = st_add / PAGESIZE;
    
    if(page_idx * PAGESIZE < st_add){
      page_idx++;
    }

    while((page_idx + 1) * PAGESIZE <= st_add + length){
